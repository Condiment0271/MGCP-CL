...
def batch_fit(batch, model, loss_func, mode):
    """ 模型 batch 处理,
    :param batch:
    :param model:
    ...
    y_pred, reps_uni, reps_mul, reps_m, reps_t, reps_v, reps_a, reps_m_aug, reps_t_aug, reps_v_aug, reps_a_aug = outputs
    y = y.view(-1)

    if mode == 'train':
        inner_product = torch.matmul(reps_uni, reps_mul.t())
        similarity = F.softmax(inner_product, dim=1)
        mask = torch.eye(batch_size)
        mask = mask.to(similarity.device)
        positive_sim = torch.diagonal(similarity, dim1=0, dim2=1)
        negative_sim = similarity * (1 - mask)
        negative_sim = negative_sim.sum(dim=1) / (batch_size - 1)
        nce_loss = -torch.log(positive_sim / (positive_sim + negative_sim)).mean()
        ...
        #  + sup_cl_loss * config["sup_cl_weight"]
        #print(regre_loss,self_cl_loss,nce_loss)
        # + self_cl_loss * config['self_cl_weight'] + nce_loss * config['nce_weight']
        loss = regre_loss
...
def train(train_loader, valid_loader, test_loader, model, regre_loss_func, scl_loss_func, optimizer, scheduler):
    ...
    for epoch in tqdm(range(config["epoch"]), desc="回归模型训练中..."):
        ...
        for batch in tqdm(train_loader):
            #with open('save/CL_features.txt', 'w') as f:
            #    f.write("")
            #with open('save/CL_features_labels.txt', 'w') as f:
            #    f.write("")
        ...
        # 保存模型
        if valid_recoder.compare_best('sum', acc2+f1-mae+corr):
            valid_recoder.update_best('sum', acc2+f1-mae+corr)
